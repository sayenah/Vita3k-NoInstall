...
ExitCode init_config(Config &cfg, int argc, char **argv, const Root &root_paths, bool portable) {
    ...
    input->add_option("--bundle", command_line.bundle_path, "Path to a Game Bundle directory to mount and boot directly (dev/testing)")
        ->default_str({})->group("Input");
    input->add_option("--play-pkg", command_line.play_pkg_path, "Path to a self-contained NoNpDrm .pkg (or .zip containing one) to play without installing (decrypts to temp, deletes on exit)")
        ->default_str({})->group("Input");
